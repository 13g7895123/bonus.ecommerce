<template>
  <div class="panel">
    <div class="panel-header">
      <span class="panel-title">第三方 API 紀錄</span>
    ...
    <div class="filter-bar">
      <select v-model="filters.service" class="f-select">
        <option value="">所有服務</option>
        <option value="twilio">Twilio</option>
        <option value="firebase">Firebase</option>
      </select>
      <input v-model="filters.action" class="f-input" placeholder="Action (e.g. sendOtp)" @keyup.enter="search" />
      <select v-model="filters.success" class="f-select">
    ...
    <div class="table-wrap">
      ...
      <table v-else class="data-table">
        <thead>
          <tr>
            <th>ID</th><th>時間</th><th>服務</th><th>Action</th>
            <th>URL</th><th>狀態碼</th><th>耗時(ms)</th><th>結果</th><th>操作</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="log in items" :key="log.id">
            ...
            <td><span :class="['badge', serviceBadge(log.service)]">{{ log.service }}</span></td>
            <td>{{ log.action }}</td>
            <td class="td-uri" :title="log.url">{{ log.url }}</td>
            <td><span :class="['badge', statusBadge(log.response_code)]">{{ log.response_code || '-' }}</span></td>
            <td class="td-num">{{ log.duration_ms }}</td>
            <td>
              <span :class="['badge', log.success == 1 ? 'badge-green' : 'badge-red']">
                {{ log.success == 1 ? '成功' : '失敗' }}
              </span>
            </td>
    ...
    <div class="pagination">
      ...
      <span class="page-info">第 {{ page }} 頁，共 {{ Math.ceil(total / limit) || 1 }} 頁（{{ total }} 筆）</span>
  ...
  <div v-if="detail.show" class="modal-overlay" @click.self="detail.show = false">
    <div class="modal-box modal-wide">
      <div class="modal-hd">
        <span>Third-Party Log #{{ detail.data?.id }} — {{ detail.data?.service }} / {{ detail.data?.action }}</span>
      ...
      <div class="modal-bd" v-if="detail.data">
        ...
        <div class="detail-row"><b>服務：</b>{{ detail.data.service }}</div>
        <div class="detail-row"><b>Action：</b>{{ detail.data.action }}</div>
        <div class="detail-row"><b>方法：</b>{{ detail.data.method }}</div>
        <div class="detail-row"><b>URL：</b><span class="url-text">{{ detail.data.url }}</span></div>
        <div class="detail-row"><b>狀態碼：</b>{{ detail.data.response_code || '-' }}</div>
        <div class="detail-row"><b>耗時：</b>{{ detail.data.duration_ms }} ms</div>
        <div class="detail-row">
          <b>結果：</b>
          <span :class="detail.data.success == 1 ? 'text-success' : 'text-error'">
            {{ detail.data.success == 1 ? '成功' : '失敗' }}
          </span>
        </div>
        <div v-if="detail.data.error_message" class="detail-row">
          <b>錯誤訊息：</b><span class="text-error">{{ detail.data.error_message }}</span>
        </div>
...
const filters = ref({ service: '', action: '', success: '', response_code: '', date_from: '', date_to: '' })
const detail  = ref({ show: false, data: null })

const buildQuery = () => {
  const p = new URLSearchParams({ page: page.value, limit })
  if (filters.value.service)       p.set('service', filters.value.service)
  if (filters.value.action)        p.set('action', filters.value.action)
  if (filters.value.success !== '') p.set('success', filters.value.success)
...
const load = async () => {
  loading.value = true
  try {
    const res  = await apiFetch(`/api/v1/sadmin/third-party-logs?${buildQuery()}`, { auth: true })
...
const clearFilters = () => { filters.value = { service: '', action: '', success: '', response_code: '', date_from: '', date_to: '' }; search() }

const openDetail = async (id) => {
  const res  = await apiFetch(`/api/v1/sadmin/third-party-logs/${id}`, { auth: true })
  const data = await res.json()
...
const serviceBadge = (s) => ({ twilio: 'badge-blue', firebase: 'badge-yellow' }[s] ?? 'badge-gray')
...
.url-text { font-family: monospace; font-size: 0.82rem; word-break: break-all; }
